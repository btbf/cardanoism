...
@rx.page(route="/auth/google/login", on_load=AuthState.start_google_login)
def google_login_redirect() -> rx.Component:
    """Googleへのリダイレクト中に表示するローディング画面。"""
    return rx.center(
        rx.vstack(
            rx.spinner(size="3"),
            rx.text("Googleへ接続中...", size="3", color="var(--gray-9)"),
            spacing="3",
            align="center",
        ),
        min_height="100vh",
...
@rx.page(route="/auth/google/callback", on_load=AuthState.handle_google_callback)
...
@rx.page(route="/auth/twitter/login", on_load=AuthState.start_twitter_login)
def twitter_login_redirect() -> rx.Component:
    """X(Twitter)へのリダイレクト中に表示するローディング画面。"""
    return rx.center(
        rx.vstack(
            rx.spinner(size="3"),
            rx.text("X(Twitter)へ接続中...", size="3", color="var(--gray-9)"),
            spacing="3",
            align="center",
        ),
        min_height="100vh",
...
@rx.page(route="/auth/twitter/callback", on_load=AuthState.handle_twitter_callback)
